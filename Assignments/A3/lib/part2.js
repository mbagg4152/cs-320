...
function main() {
    ...
    config_draw(gl2, TRI, can2, v4_black, src_vert, grad_frag, GRAD);
    config_draw(gl3, CIRC, can3, v4_red, src_vert, rad_frag, RAD);
    config_draw(gl4, SQ0, can4, v4_black, src_vert, src_frag, BASIC);
}
...
function config_draw(gl, shape, canvas, color_vals, v_shader, f_shader, grad_type) {
    ...
    try { v_count = init_buffers(gl, shape); } catch (e) { clog('BUFF ERR\n' + e); }
    ...
    if (shape === 'S') {
        ...
        simple_color_vert_update(gl, SQ1, v4_white);
        simple_color_vert_update(gl, SQ2, v4_black);
        simple_color_vert_update(gl, SQ3, v4_white);
        simple_color_vert_update(gl, SQ4, v4_black);

    } else if (shape === 'T') gl.drawArrays(gl.TRIANGLES, 0, v_count);
    else if (shape === 'C' || shape === 'E') gl.drawArrays(gl.TRIANGLE_STRIP, 0, v_count);
}

function simple_color_vert_update(gl, shape, color) {
    try { uni_color = gl.getUniformLocation(gl.program, 'l_color'); } catch (e) { clog('COLOR ERR\n' + e); }
